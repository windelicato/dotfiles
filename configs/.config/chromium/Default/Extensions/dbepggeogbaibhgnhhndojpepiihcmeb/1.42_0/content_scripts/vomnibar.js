...
(function() {
  ...
  VomnibarUI = (function() {
    ...
    VomnibarUI.prototype.onKeydown = function(event) {
      ...
      openInNewTab = this.forceNewTab || (event.shiftKey || KeyboardUtils.isPrimaryModifierKey(event));
      if (action === "dismiss") {
        this.hide();
      } else if (action === "up") {
        this.selection -= 1;
  ...
  BackgroundCompleter = (function() {
    ...
    function BackgroundCompleter(name) {
      this.name = name;
      this.filterPort = chrome.extension.connect({
    ...
    completionActions: {
      navigateToUrl: function(url, openInNewTab) {
        if (url.startsWith("javascript:")) {
          return eval(decodeURIComponent(url.slice("javascript:".length)));
        } else {
          return chrome.extension.sendRequest({
            handler: openInNewTab ? "openUrlInNewTab" : "openUrlInCurrentTab",
            url: url,
